 ...
 */
static void
send_instruction(struct twsfw_ctl *ctl)
{
	FILE *fp;
	int byte_count;
...
view_rules(void)
{
	...
	while((byte_count = fread(buffer, 1, sizeof(struct twsfw_rule), fp)) > 0) {
		rule = (struct twsfw_rule *)buffer;
 ...
 */
static int
parse_arguments(int argc, char **argv, struct twsfw_ctl *ret_ctl)
{
	int opt;
	int64_t lnum;
	int opt_index;
	...
	if(ctl.mode != TWSFW_VIEW && ctl.rule.in == -1) {
